<template>
	<view class="singlePlayer" @click="move">
		...
		<view class="bottom">
			...
			<view class="your">电脑</view>
		</view>
		<!-- 选择先手弹出层 -->
		<uni-popup ref="popup" :mask-click="false" type="center">
			<view class="choose-item">
				<button @click="julioCruz(1)">
					<image class="choose-img" src="/static/img/my-bout.png"></image>
				</button>
				<button @click="julioCruz(2)">
					<image class="choose-img" src="/static/img/computer-bout.png"></image>
				</button>
			</view>
		</uni-popup>
	</view>
	...
	export default {
		...
		methods: {
			...
			algorithm() {
				...
				const arr = JSON.stringify(this.yourDice)
				// 循环计算每个位置的分数期望
				for (let i = 0; i < this.yourDice.length; i++) {
					// 转回数组
					let newDice = JSON.parse(arr)
					if (parseInt(newDice[i]) == 0) {
						newDice[i] = this.yourPoint
						// js除出来是浮点数,需用 parseInt取整数
						const x = parseInt(i / 3)
						// 拷贝我方棋盘
						let myArr = JSON.parse(JSON.stringify(this.myDice))
						// 消除棋子
						for (let a = 0; a < 3; a ++) {
						...
						// 消除结束计算分数差
						let poor = this.getScore(newDice) - this.getScore(myArr)
						score[i] = poor
					} else {
				...
				// 在该数组随机一个位置填入
				let rand = this.rand(0, maxArr.length - 1)
				let position = maxArr[rand]
				this.OPlay = true
